	...
	outputdir=$( cat $runfile | grep -w OUTPUTDIR | cut -d '=' -f2 )
		...
		then
		    ...
		    echo "#PBS -W depend=afterok:$CONVERT" >> $qsub
                    if [ $paired -eq 1 ]
                    then
			echo "$scriptdir/novosplit.sh $alignerdir $alignparms $refdir/$refindexed $outputalign $outputsam.node$i.sam $scriptdir $paired $outputalign/$Rone $outputalign/$Rtwo $outputlogs/log.novo.node$i.in $outputlogs/log.novo.node$i.ou $email $outputlogs/qsub.novo.node.$i" >> $qsub
                    else
			echo "$scriptdir/novosplit.sh $alignerdir $alignparms $refdir/$refindexed $outputalign $outputsam.node$i.sam $scriptdir $paired $outputalign/$Rone $outputlogs/log.novo.node$i.in $outputlogs/log.novo.node$i.ou $email $outputlogs/qsub.novo.node.$i" >> $qsub
                    fi
                    `chmod a+r $qsub`
                    jobnovo=`qsub $qsub`
			...
			echo "$scriptdir/bwaS2.sh $alignerdir $refdir/$refindexed $outputalign $outputsam.node$i.R1.sai $outputsam.node$i.R2.sai $outputalign/$Rone $outputalign/$Rtwo $outputsam.node$i.sam $outputlogs/log.bwar3.node$i.in $outputlogs/log.bwar3.node$i.ou $email $outputlogs/qsub.bwar3.node.$i" >> $qsub3
			`chmod a+r $qsub3`
                        jobwa=`qsub $qsub3`
			...
			echo "$scriptdir/bwaS3.sh $alignerdir $refdir/$refindexed $outputalign $outputsam.node$i.R1.sai $outputalign/$Rone $outputsam.node$i.sam $outputlogs/log.bwar3.node$i.in $outputlogs/log.bwar3.node$i.ou $email $outputlogs/qsub.bwar3.node.$i" >> $qsub3
			`chmod a+r $qsub3`
                        jobwa=`qsub $qsub3`
                ...
                echo "step 2: sort chunk"
                if [ `expr length $jobnovo` -gt 0 ]
                then
		    jobaln=$( echo $jobnovo )
                else
                    jobaln=$( echo $jobwa )
                ...
                qsub4=$outputlogs/qsub.sort.node.$i
                echo "#PBS -V" > $qsub4
                echo "#PBS -A $pbsprj" >> $qsub4
                echo "#PBS -N sortnode_$i" >> $qsub4
		echo "#PBS -l walltime=$pbscpu" >> $qsub4
		echo "#PBS -l nodes=1:ppn=16" >> $qsub4
		echo "#PBS -o $outputlogs/log.sort.node$i.ou" >> $qsub4
		echo "#PBS -e $outputlogs/log.sort.node$i.in" >> $qsub4
                echo "#PBS -q $pbsqueue" >> $qsub4
                echo "#PBS -m ae" >> $qsub4
                echo "#PBS -M $email" >> $qsub4
		echo "#PBS -W depend=afterok:$jobaln" >> $qsub4
                echo "$scriptdir/sortnode.sh $picardir $samdir $outputalign $outputsam.node$i.sam $outputsam.node$i.wrg.sorted.bam $RGparms NCSA $outputlogs/log.sort.node$i.in $outputlogs/log.sort.node$i.ou $email $outputlogs/qsub.sort.node.$i" >> $qsub4
                `chmod a+r $qsub4`
                `qsub $qsub4 >> $outputlogs/SORTEDpbs`

                allfiles=$allfiles" INPUT=$outputsam.node$i.wrg.sorted.bam  "
		echo `date`
            done

            ## merge chunks into a single bam file
            ## then mark and or remove duplicates

	    echo `date`
            echo "step 3: merging chunks for sample $dirname"

	    SORTED=$( cat $outputlogs/SORTEDpbs | sed "s/\.[a-z]*//" | tr "\n" ":" )
	    ...
	    echo "#PBS -e $outputlogs/log.mergenodes.in" >> $qsub1
            ...
            echo "$scriptdir/mergenodes.sh $outputalign $listfiles $outsortwdup $outsortnodup $sortedplain $dupparms $runfile $outputlogs/log.mergenodes.in $outputlogs/log.mergenodes.ou $email $outputlogs/qsub.merge.nodes" >> $qsub1
            `chmod a+r $qsub1`
            mergejob=`qsub $qsub1`
